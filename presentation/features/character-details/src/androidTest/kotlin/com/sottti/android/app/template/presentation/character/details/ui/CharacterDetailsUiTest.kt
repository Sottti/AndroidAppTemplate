...
internal class CharacterDetailsUiTest(
    ...
    fun givenStateIsInitialLoad_thenFullScreenProgressBarsAreVisible() =
        runUiTest(content = { CharacterDetailsUiPreview(loadingState) }) {
            onNode(hasTestTag(PROGRESS_INDICATOR_TEST_TAG)).assertIsDisplayed()
            onNodeWithTag(ERROR_TEST_TAG).assertDoesNotExist()
            onNodeWithTag(CHARACTER_DETAILS_DETAILS_TEST_TAG).assertDoesNotExist()
            onNodeWithTag(CHARACTER_DETAILS_IMAGE_TEST_TAG).assertDoesNotExist()
            onNodeWithTag(MAIN_TOP_BAR_TEST_TAG).assertIsDisplayed()
        }

    @Test
    fun givenStateIsError_thenErrorIsVisible() =
        runUiTest(content = { CharacterDetailsUiPreview(errorState) }) {
    ...
    fun givenStateIsLoaded_thenImageAndDetailsAreVisible() =
        runUiTest(content = { CharacterDetailsUiPreview(loadedState) }) {
            onNode(hasTestTag(PROGRESS_INDICATOR_TEST_TAG)).assertDoesNotExist()
            onNodeWithTag(ERROR_TEST_TAG).assertDoesNotExist()
            onNodeWithTag(CHARACTER_DETAILS_DETAILS_TEST_TAG).assertIsDisplayed()
            onNodeWithTag(CHARACTER_DETAILS_IMAGE_TEST_TAG).assertIsDisplayed()
            onNodeWithTag(MAIN_TOP_BAR_TEST_TAG).assertIsDisplayed()
        }
}
