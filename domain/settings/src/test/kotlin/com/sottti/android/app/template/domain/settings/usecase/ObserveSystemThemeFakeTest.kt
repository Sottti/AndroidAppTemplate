...
internal class ObserveSystemThemeFakeTest {

    @Test
    fun `when created, then it provides a flow that emits the initial value`() = runTest {
        val initialValue = SystemTheme.LightSystemTheme
        val fake = ObserveSystemThemeFake(initialValue)
    ...
    fun `when emit is called, then the flow emits the new value`() = runTest {
        val fake = ObserveSystemThemeFake(initialValue = SystemTheme.LightSystemTheme)
        val newValue = SystemTheme.DarkSystemTheme
