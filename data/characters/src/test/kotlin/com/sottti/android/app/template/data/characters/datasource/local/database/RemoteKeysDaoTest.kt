...
internal class RemoteKeysDaoTest {
    ...
    fun `upsert inserts keys and get returns them`() = runTest {
        val keysToInsert = RemoteKeysRoomModel(anchor = "characters", nextPage = 3, prevPage = 1)
        ...
        assertThat(retrievedKeys?.anchor).isEqualTo("characters")
    }

    @Test
    fun `upsert with same anchor replaces existing keys`() = runTest {
        val initialKeys = RemoteKeysRoomModel(anchor = "characters", nextPage = 3, prevPage = 1)
        dao.upsert(initialKeys)

        val updatedKeys = RemoteKeysRoomModel(anchor = "characters", nextPage = 4, prevPage = 2)
    ...
    fun `get uses default anchor and retrieves correct keys`() = runTest {
        val keysToInsert = RemoteKeysRoomModel(anchor = "characters", nextPage = 5, prevPage = 3)
        dao.upsert(keysToInsert)
    ...
    fun `get with different anchor returns null`() = runTest {
        ...
        val retrievedKeys = dao.get(anchor = "other_characters")
    ...
    fun `upsert allows null prev or next`() = runTest {
        dao.upsert(RemoteKeysRoomModel(anchor = "characters", prevPage = null, nextPage = 2))
        val a = dao.get()
        assertThat(a?.prevPage).isNull()
        assertThat(a?.nextPage).isEqualTo(2)

        dao.upsert(RemoteKeysRoomModel(anchor = "characters", prevPage = 1, nextPage = null))
        val b = dao.get()
    ...
    fun `upsert with same anchor keeps single row`() = runTest {
        dao.upsert(RemoteKeysRoomModel("characters", nextPage = 2, prevPage = 0))
        dao.upsert(RemoteKeysRoomModel("characters", nextPage = 3, prevPage = 1))
        val count = db
    ...
    fun `withTransaction executes block`() = runTest {
        dao.withTransaction {
            dao.upsert(RemoteKeysRoomModel("characters", nextPage = 10, prevPage = 9))
        }
